            ...
            (Some(node1_ptr), Some(node2_ptr)) => {
                unsafe {
                    ...
                    if node1.prev == *node2_ptr {
                        ...
                        mem::swap(&mut (*(node1).prev).next, &mut (*(node2).next).prev);
                    }
                    else if node2.prev == *node1_ptr {
                        node1.next = node2.next;
                        node2.next = *node1_ptr;

                        node2.prev = node1.prev;
                        node1.prev = *node2_ptr;

                        mem::swap(&mut (*(node1).next).prev, &mut (*(node2).prev).next);
                    }
                    else {
